...
def create_llm():
    ...
    """
    raise RuntimeError("Use create_llm_with_args()")
...
def create_graph(llm):
    ...
    def get_user_input(state: AgentState) -> dict:
        ...
        # Toggle tracing mode based on special commands.
        # We still pass the input through to the LLM, but downstream nodes
        # will print tracing information depending on this flag.
        ...
        if tracing:
            print(f"[TRACE] enter get_user_input user_input={user_input!r} (tracing now on)")
    ...
    def call_llm(state: AgentState) -> dict:
        ...
        if state.get("tracing", False):
            print(f"[TRACE] enter call_llm user_input={user_input!r}")
    ...
    def print_response(state: AgentState) -> dict:
        ...
        """
        if state.get("tracing", False):
            print("[TRACE] enter print_response")
        print("\n" + "-" * 50)
        print("LLM Response:")
        print("-" * 50)
    ...
    def route_after_input(state: AgentState) -> str:
        ...
        """
        if state.get("tracing", False):
            print(f"[TRACE] route_after_input should_exit={state.get('should_exit', False)}")
        # Check if user wants to exit
        if state.get("should_exit", False):
            return END

        # Never pass empty input to the LLM: route back to get_user_input.
    ...
    graph_builder.add_conditional_edges(
        ...
        {
            "get_user_input": "get_user_input",  # Empty input -> prompt again
...
def main():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Skip loading the model and return a dummy response (for quick graph/testing).",
    )
    args = parser.parse_args()

    # Step 1: Create and configure the LLM
    llm = create_llm_with_args(model_id=args.model_id, dry_run=args.dry_run)
    ...
    save_graph_image(graph, filename=str(out_dir / "task2_graph.png"))
